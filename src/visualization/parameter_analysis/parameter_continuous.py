...
PARAMETER_RANGES = {
    'PSO': {
        'w': np.round(np.arange(0.1, 1.0, 0.1), 2),
        'c1': np.round(np.arange(0.5, 2.5, 0.2), 2),
        'c2': np.round(np.arange(0.5, 2.5, 0.2), 2),
    },
    'ABC': {
        'limit': np.round(np.arange(10, 100, 10), 0).astype(int),
    },
    'FA': {
        'alpha': np.round(np.arange(0.2, 1.0, 0.1), 2),
        'beta': np.round(np.arange(0.1, 1.5, 0.1), 2),
        'gamma': np.round(np.arange(0.01, 0.2, 0.01), 2),
    },
    'Cuckoo': {
        'pa': np.round(np.arange(0.1, 0.9, 0.05), 2),
        'alpha': np.round(np.arange(0.01, 0.5, 0.05), 2),
        'beta': np.round(np.arange(0.3, 2.0, 0.1), 2),
    },
    'ACO': {
        'rho': np.round(np.arange(0.1, 0.99, 0.05), 2),
    }
}
...
def create_algorithm_with_parameter(algo_name, param_name, param_value, seed):
    """Create algorithm instance with specific parameter value.

    Parameters:
    algo_name (str): Name of the algorithm (FA, ABC, Cuckoo, PSO, ACO)
    param_name (str): Name of the parameter to set
    param_value (float): Value of the parameter to set
    seed (int): Random seed for reproducibility

    Returns:
    object: Algorithm instance configured with the specified parameter
    """
    if algo_name == 'FA':
        if param_name == 'alpha':
...
def run_parameter_experiment(algo_name, param_name, param_value, seed):
    """Run algorithm once with specific parameter value and return best fitness.

    Parameters:
    algo_name (str): Name of the algorithm to run
    param_name (str): Name of the parameter being tested
    param_value (float): Value of the parameter to test
    seed (int): Random seed for reproducibility

    Returns:
    float: Best fitness value found, or None if error occurred
    """
    np.random.seed(seed)
    random.seed(seed)
    try:
...
def analyze_parameter(algo_name, param_name, param_values):
    """Analyze one parameter for one algorithm across multiple runs.

    Parameters:
    algo_name (str): Name of the algorithm to analyze
    param_name (str): Name of the parameter to analyze
    param_values (list): List of parameter values to test

    Returns:
    list: List of dictionaries containing analysis results with avg_fitness and std_fitness
    """
...
def plot_parameter_analysis(df, save_dir='src/visualization/parameter_analysis'):
    ...
    Parameters:
    df (pandas.DataFrame): DataFrame containing parameter analysis results
...
def run_all_parameter_analysis():
    """Run parameter analysis for all swarm algorithms on continuous problem.

    Parameters:
    None

    Returns:
    pandas.DataFrame: DataFrame containing all parameter analysis results
    """
    all_results = []
    print("=" * 80)
